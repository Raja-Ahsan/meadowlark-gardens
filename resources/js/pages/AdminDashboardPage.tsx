...
import { AdminStats, Order, Product, WholesaleApplication } from '@/types'
...
export default function AdminDashboardPage() {
  ...
  const loadData = async () => {
    try {
      const [statsRes, appsRes, productsRes, ordersRes] = await Promise.all([
        api.getAdminStats(),
  ...
  const handleLogout = async () => { await logout(); navigate('/wholesale/login') }
  ...
  const statCards = stats ? [
    { icon: DollarSign, label: 'Total Revenue', value: `$${stats.totalRevenue.toLocaleString()}`, color: 'bg-forest-100 text-forest-600' },
    { icon: ShoppingBag, label: 'Total Orders', value: String(stats.totalOrders), color: 'bg-blue-100 text-blue-600' },
    { icon: ClipboardList, label: 'Pending Applications', value: String(stats.pendingApplications), color: 'bg-amber-100 text-amber-600' },
    { icon: Users, label: 'Active Wholesalers', value: String(stats.activeWholesalers), color: 'bg-sage-100 text-sage-700' },
  ] : []
                  ...
                  {statCards.map((stat, i) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className={`text-xs font-sans font-600 px-2 py-0.5 rounded-full bg-forest-50 text-forest-600`}>
                          Live
                        </span>
                      </div>
                    ...
                    <div className="space-y-3">
                      {orders.slice(0, 5).map(order => (
                        <div key={order.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                          <div>
                            <p className="font-sans font-600 text-forest-800 text-sm">{order.orderNumber}</p>
                          ...
                          <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-4 py-3">
                              <div className="flex gap-2">
                                ...
                                <button onClick={() => deleteProduct(product.id)} className="p-1.5 text-gray-400 hover:text-terra-600 hover:bg-terra-50 rounded-lg transition-colors" aria-label="Delete">
            ...
            {activeTab === 'orders' && (
              <motion.div key="orders" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }}>
                <div className="mb-6">
                  ...
                  <p className="text-gray-500 text-sm font-body mt-0.5">{orders.length} total orders</p>
                </div>
