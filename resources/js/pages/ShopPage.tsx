...
export default function ShopPage() {
  ...
  useEffect(() => {
    api.getProducts({ category: activeCategory, search })
  ...
  const handleCheckout = async () => {
    const customerName = window.prompt('Your name:')
    if (!customerName) return
    const customerEmail = window.prompt('Your email:')
    if (!customerEmail) return

    setCheckingOut(true)
    try {
      const { message } = await api.placeRetailOrder({
        customerName,
        customerEmail,
              ...
              {cart.length > 0 && (
                <div className="p-6 border-t border-forest-100">
                  ...
                  <button
                    ...
                    className="w-full py-3.5 bg-forest-600 hover:bg-forest-700 disabled:bg-forest-400 text-white font-sans font-700 rounded-xl transition-colors focus-ring"
                  >
